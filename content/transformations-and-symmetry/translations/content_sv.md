...
Många geometriska koncept, som [linjer](gloss:line) and [punkter](gloss:point),
"uppfanns" av matematiker. Symetri, å andra sidan, är överallt omkring oss.
Nästan alla växter, djur och till och med människor är symetriska.
...
Med tiden har vi imiterat naturens symetri i konst, arkitektur, teknik och design.
Symmetriska former och mönster verkar helt enkelt _snyggare_ än icke symetriska.
...
Men symetri är mycket viktigare än att bara vara _snyggt_. Det är en av grundstenarna
i vårt universum och kan till och med förklara dom mest fundamentala fysiska lagarna. 

Även om symetri är ett väldigt intuitivt koncept, så är det mycket svårare att beskriva 
matematiskt än man kan tro. Till att börja med måste vi lära oss om _transformationer_.
...
En [__transformation__](gloss:transformation) är en samling regler som 
konverterar en geometrisk figur till en annan. Här är några exempel:
...
{.reveal(when="t1 t2 t3")} Resultatet av transformationen kallas en
[__avbild__](gloss:transformation-image). Avbilden av figur `A` brukar 
vanligtvis skrivas som `A'` (uttalas “A prim”).

---
> id: rigid

Till att börja med kommmer vi bara tänka på transformationer som inte 
ändrar ursprungsfigurens storlek eller form. Tänk att den är gjord av
ett material om trä eller metall: vi kan flytta, vrida och vända den, men
vi kan inte böja eller på andra sätt deformera den. Dessa transformationer
kallas för [__stela transformationer__](gloss:rigid-transformation).

Vilken av dessa transformationer är stela?
...
För stela transformationer är bilden alltid
For rigid transformations, the image is always
[[kongruent med|samma som|spegelvänd mot]] ursprungsfiguren. Det finns tre olika
typer av stela transformationer:
...
{.text-center} En transformation som bara _förflyttar_ en figur kallas för
[__parallellförflyttning__](gloss:translation).
...
{.text-center} En transformation som speglar en figur kallas för 
[__spegling__](gloss:reflection).
...
{.text-center} En transformation som snurrar en figur kallas för en
